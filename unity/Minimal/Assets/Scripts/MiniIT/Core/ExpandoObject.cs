...
namespace MiniIT
{
	public class ExpandoObject : IDisposable // ICloneable
	{
		...
		public void Dispose()
		{
			IEnumerable keys = new List<string>(this.GetDynamicMemberNames());  // copy of keys list for "out of sync" exception workaround
			foreach (string key in keys)
			{
				object member = this[key];
		...
		///      
		public bool TrySetMember(string field_name, object value)
		{
			if (!mMembers.ContainsKey(field_name))
				mMembers.Add(field_name, value);
			else
				mMembers[field_name] = value;
		...
		///       
		public bool TryGetMember(string field_name, out object result)
		{
			if (mMembers.ContainsKey(field_name))
			{
				result = mMembers[field_name];
				return true;
			}
		...
		public bool TryGetMember<T>(string field_name, ref T result)
		{
			object res;
			if (TryGetMember(field_name, out res))
		    {
				try
				{
					result = (T)res;
...
//		}
		...
		{
			return mMembers.ContainsKey(name);
		}

		//-------------------

		/// <summary>
		/// Gets or sets the <see cref="MiniIT.Net.ExpandoObject"/> with the specified key.
		/// </summary>
		/// <param name="key">Name of the property being get or set</param>
		public object this[object key]
		{
			get
			{
				object result;
				this.TryGetMember((string)key, out result);
				return result;
			}

			set
			{
				this.TrySetMember((string)key, value);
			}
		}
		...
		protected static void ConvertToJSONString(object obj, ref StringBuilder string_builder)
		{
			...
			if (obj is ExpandoObject)
			{
				...
				add_comma = false;
				foreach (string key in ((ExpandoObject)obj).mMembers.Keys)
				{
					if (add_comma)
						string_builder.Append(",");
					else
					...
					object item = ((ExpandoObject)obj).mMembers[key];
			...
			{  
				string_builder.Append( Convert.ToString(obj).ToLower() );
			}
			else
			{
		...
		private object parseValue()
		{
			...
			switch ( token.type )
			{
				...
				case JSONTokenType.LEFT_BRACKET:
					return parseArray();  // ArrayList actually
	...
	class JSONTokenizer
	{
		...
		private JSONToken readNumber()
		{
			...
			float float_num = Convert.ToSingle(input);
